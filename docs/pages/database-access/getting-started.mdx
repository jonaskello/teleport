...
<Detail title="If you are setting up Teleport Application Access">
If you plan to set up Teleport Application Access, you will also need to create a DNS A record for `*.teleport.example.com` so Let's Encrypt can assign a wildcard certificate and enable access to all of your applications. This is because Teleport assigns a subdomain to each application you have configured for Application Access, e.g., `grafana.teleport.example.com`.
