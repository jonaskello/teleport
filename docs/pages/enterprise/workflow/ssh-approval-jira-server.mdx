...
First, using an existing Teleport Cluster, create a new user and role to access Teleport.
...
# ...
```
The above sequence should generate an identity file called `auth.pem`. We'll reference this file later when [configuring the plugins](#configuration-file).
...
<Tabs>
<TabItem title="Self Hosted" scope={["oss", "enterprise"]}>
```toml
(!examples/resources/plugins/teleport-jira-self-hosted.toml!)
```
</TabItem>
<TabItem title="Self Hosted" scope={["cloud"]}>
```toml
